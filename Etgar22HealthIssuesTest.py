...
try:
    ...
    try:
        sleep(1)
        facebook_name_box = driver.find_elements_by_xpath('//input[@placeholder = "שם מלא ושם בפייסבוק"]')
...
sleep(380)  # wait for the sign ups to insert in the sign ups form
...
row_failed = [time_now, "אתגר 22 מבוגרים עם בעיות בריאות", email, "Sign up failed: not found in the sheet", random_issue, random_time_pass]
row_succeed = [time_now, "אתגר 22 מבוגרים עם בעיות בריאות", email, "Sign up succeed", random_issue, random_time_pass]
